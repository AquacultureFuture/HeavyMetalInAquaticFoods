...
# 读取数据
df <- read.csv("D:/R Graph Data/R Graph/Cadmium_all.csv")

# 设置随机种子，使结果能够重现
set.seed(123)

# 运行随机森林
rf_results <- randomForest(Cadmium ~ ., data = df, importance = TRUE, ntree = 500)
...
# 自定义顺序列表
custom_order <- c("MarineFreshwater", "FeedingPath", "ClimateZone", "DemersalPelagic", "DepthRangeDeep", "K", "FishingVulnerabilityScore", "tm", "Resilience", "Lmax", "TrophicLevel")
# 创建行名列
...
custom_order <- c("TrophicLevel", "Lmax", "Resilience", "tm", "FishingVulnerabilityScore", "K", "DepthRangeDeep", "DemersalPelagic", "ClimateZone", "FeedingPath", "MarineFreshwater")
...
# 保存图像为SVG格式
ggsave("Cadmium_all_plots.svg", plot = combined_plot, width = 12, height = 8, device = "svg")

# 计算 MAE, RMSE 和 R²
predictions <- predict(rf_results, df)
mae <- mean(abs(predictions - df$Cadmium))
rmse <- sqrt(mean((predictions - df$Cadmium)^2))
r2 <- cor(predictions, df$Cadmium)^2
