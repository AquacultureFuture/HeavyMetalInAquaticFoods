...
# 读取数据
df <- read.csv("D:/R Graph Data/R Graph/Mercury_all.csv")

# 设置随机种子，使结果能够重现
set.seed(123)

# 运行随机森林
rf_results <- randomForest(Mercury ~ ., data = df, importance = TRUE, ntree = 500)
...
# 保存图像为SVG格式
ggsave("Mercury_all_plots.svg", plot = combined_plot, width = 12, height = 8, device = "svg")

# 计算 MAE, RMSE 和 R²
predictions <- predict(rf_results, df)
mae <- mean(abs(predictions - df$Mercury))
rmse <- sqrt(mean((predictions - df$Mercury)^2))
r2 <- cor(predictions, df$Mercury)^2
