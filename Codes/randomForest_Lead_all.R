...
# 读取数据
df <- read.csv("D:/R Graph Data/R Graph/Lead_all.csv")

# 设置随机种子，使结果能够重现
set.seed(123)

# 运行随机森林
rf_results <- randomForest(Lead ~ ., data = df, importance = TRUE, ntree = 500)
...
# 保存图像为SVG格式
ggsave("Lead_all_plots.svg", plot = combined_plot, width = 12, height = 8, device = "svg")

# 计算 MAE, RMSE 和 R²
predictions <- predict(rf_results, df)
mae <- mean(abs(predictions - df$Lead))
rmse <- sqrt(mean((predictions - df$Lead)^2))
r2 <- cor(predictions, df$Lead)^2
